...
const String shortLorem =
    'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer convallis ipsum tortor, cursus volutpat sapien iaculis ac. Vivamus eu dui pharetra, accumsan eros non, placerat sem.';
const String longLorem =
...
TextInputFormatter textFormatterDoubleWithNegative =
    FilteringTextInputFormatter.allow(RegExp(r'^-?\d+\.?\d*$'));
TextInputFormatter textFormatterDoubleOnly = FilteringTextInputFormatter.allow(
  RegExp(r'^\d+\.?\d*$'),
);
TextInputFormatter textFormattedDigitsWithNegative =
    FilteringTextInputFormatter.allow(RegExp(r'^-?\d+'));
TextInputFormatter textFormatterDigitsOnly =
    FilteringTextInputFormatter.digitsOnly;
TextInputFormatter textFormatterSingleLine =
    FilteringTextInputFormatter.singleLineFormatter;
TextInputFormatter textFormatterLetterOnly = FilteringTextInputFormatter.allow(
  RegExp(r'[a-zA-Z]'),
);
TextInputFormatter textFormatterLetterDigitsOnly =
...
EdgeInsets responsivePadding(
  Size size, {
  double minimumHorizontalPadding = 16.0,
}) {
  ...
  if (size.width <= kMediumSize) {
    return EdgeInsets.symmetric(
      horizontal: (size.width - kCompactSize) / 2 + minimumHorizontalPadding,
    );
  }

  return EdgeInsets.symmetric(
    horizontal: (size.width - kMediumSize) / 2 + minimumHorizontalPadding,
  );
}
...
Color selectedTileColor({Animation<double>? animation}) => animation != null
    ? Color.lerp(
        Colors.transparent,
        Theme.of(navigatorKey.currentContext!).colorScheme.secondaryContainer,
        animation.value,
      )!
    : Theme.of(navigatorKey.currentContext!).colorScheme.secondaryContainer;
...
Future<void> animateScrollController({
...
Future<void> animateDraggableScrollableController({
...
Future<void> animatePageController({
  required int page,
...
String jsonToCorrectStringFormat(String jsonString) {
  ...
  while (words.characters.elementAt(0) == '\'' ||
      words.characters.elementAt(0) == '"') {
    words = words.substring(1);
  }
  while (words.characters.elementAt(words.length - 1) == '\'' ||
      words.characters.elementAt(words.length - 1) == '"') {
